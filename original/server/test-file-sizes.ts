...
async function testFileSize(client: any, charCount: number): Promise<string> {
  ...
  try {
    ...
    const msg = e.message?.includes("503") ? "503 SERVICE_UNAVAILABLE" : 
                e.message?.includes("400") ? "400 INVALID_REQUEST" : e.message;
    return `FAILED: ${msg}`;
  } finally {
...
async function main() {
  ...
  // 再確認: 小さいファイルで成功するか
  console.log("Test 1: 100 chars");
  console.log(await testFileSize(client, 100));
  
  await sleep(65000);
  
  // 1000文字
  console.log("\nTest 2: 1000 chars");
  console.log(await testFileSize(client, 1000));
}
